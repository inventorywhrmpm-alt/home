...
ticker_input = st.sidebar.text_input("Masukkan Ticker IDX (Contoh: BBCA, BBRI, TLKM)", "SCMA, BBCA")
...
if ticker_input:
    ...
    for ticker in tickers:
        ...
        try:
            # Menggunakan auto_adjust=False agar data Open/High/Low/Close sama dengan chart reguler TradingView
            ...
            # Memastikan tipe data berupa float agar kalkulasi presisi
            ...
            if psar is not None and not psar.empty:
                ...
                # Memasukkan kode ticker asli ke DataFrame
                df['Ticker'] = ticker
                
                # Format Tanggal menjadi string YYYY-MM-DD
                df['Tanggal'] = df['Date'].dt.strftime('%Y-%m-%d')
                
                # Mengambil x hari terakhir sesuai pilihan slider setelah SAR selesai dihitung
                df_filtered = df.tail(display_days)
                
                # Pilih kolom sesuai spesifikasi urutan permintaan
    ...
    if all_data:
        ...
        # Memastikan kolom angka bertipe numerik untuk formatting desimal
        ...
        st.warning("Tidak ada data yang berhasil diambil. Pastikan kode saham IDX yang dimasukkan sudah benar.")
